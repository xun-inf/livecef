...
#include <Shlwapi.h>
#include "../../browser/CefAppDelegate.h"
#undef NO_SHLWAPI_ISOS

#include <QDebug>
#include <QDir>

bool CefContextPrivate::initializeCef(const CefConfig* config)
{
  ...
  CefConfigPrivate::CopyToCefSettings(config, &cef_settings);
  ...
  // path values
  QString strExePath = QDir(QString::fromWCharArray(modPath.data())).filePath(kRenderProcessName);
  ...
  // Initialize CEF
  auto cmdArgs = CefConfigPrivate::GetCommandLineArgs(config);
  cmdArgs[kWindowsJobNameKey] = windowsJobName_.toStdString();
  auto appDelegate = std::make_shared<CefAppDelegate>(this, cmdArgs);
  ...
  auto app = new CefBrowserApp(std::string(), appDelegate);
...
void CefContextPrivate::uninitializeCef()
{
  if (!pApp_)
    return;
